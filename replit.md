...
- **Responsive Navigation**: A professional sidebar component offers collapsible/expandable states, user profile display, dark/light mode toggle with persistence, and navigation links. A top navigation bar provides ticket search functionality with welcome message, two mutually exclusive search input fields (Search Ticket Number and Partner Ticket Number - one disables when the other has text), search button, and "+ New Ticket" button, all styled with brand colors (orange gradient background #ee754e to #f49b71).
- **Ticket Search & Details**: Implements comprehensive ticket search via API (`/api/v1/tickets/search?ticket_number={value}`). When a ticket is found, opens a new tab at `/ticket-details/{ticketId}` displaying complete ticket information fetched from `/api/v1/tickets/{id}`. The details page features Autotask-style 3-column layout with: (1) Left sidebar (200px) - Company/Contact tags, Status/Priority dropdowns, collapsible Ticket Information, Assignment, Billing, User-Defined Fields sections; (2) Main content (flex-1) - Description with rich text toolbar, Timeline with interactive milestones, Resolution, Checklist with New Item/Add from Library/Save to Library, Activity tabs (Activity, Attachments, Charges & Expenses, Service Calls & To-Dos); (3) Right sidebar (220px) - Company/Contact info, Time Summary with progress bar, Configuration Item, Company section. Features edit mode toggle with Save/Save & Close/Cancel toolbar, all sidebar fields become editable (dropdowns, tag inputs with search/add buttons, text inputs, date+time pickers). Image attachments display fully stretched in notes. New Attachment modal with Type dropdown, File upload with image preview, Name field.
